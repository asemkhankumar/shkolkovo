...
//        $this->set('_return', $rr . Url::query());
        ...
        /*
        * Версия (обычная-слабовидящих)
        * false - обычная
        * true - для слабовидящих
        */
        /*if (isset($_COOKIE['version_site']))
        {
            $version_for_see = true;

            $this->default_template = $this->request->controller().'/'.$this->request->action().'special';

            if ($this->request->directory())
            {
                /* если есть дирректория в запросе */
                /*$this->default_template = $this->request->directory().'/'.$this->default_template;
            }

           $this->default_template = strtolower($this->default_template);
        }
        else
        {
            $version_for_see = false;
        }
        $this->set('version_for_see',$version_for_see);*/

       /* if (isset($_COOKIE['vision']))
        {
            $version_for_see = 'v2';
//            Cookie::delete('vision');
//            Cookie::set('vision', $version_for_see);
        }
        else {
            $version_for_see= 'false';
        }
        ...
        if (isset($_COOKIE['vision1']))
        {
            $version_for_see1 = 'v3';
//            Cookie::delete('vision1');
//            Cookie::set('vision1', $version_for_see1);
        }
        else {
            $version_for_see1= 'false';
        }
        // $version_for_see = 1;
        $this->set('version_for_see1',$version_for_see1);*/
		$version_for_see = 'v1';
if (isset($_COOKIE['vision']))
        { $version_for_see = 'v2';
	//Cookie::delete('vision');
...
if (isset($_COOKIE['vision1']))
		...
		$version_for_see = 'v3';
			//Cookie::delete('vision1');
        // Cookie::set('vision1', $version_for_see);}
	}
$this->set('version_for_see',$version_for_see);
//        if (!isset ($_COOKIE['vision']) and !isset ($_COOKIE['vision1']))
//        {
//            $version_for_see2= 'true';
//        }
//        else {
//            $version_for_see2= 'false';
//        }
//        $this->set('version_for_see2',$version_for_see2);


        // $journal=ORM::factory('Calling')->order_by('date','DESC')->limit(1)->find();
